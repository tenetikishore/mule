...
public class CompositeArtifactExtensionManagerTestCase extends AbstractMuleTestCase {
  ...
  private final ConfigurationModel configurationModel = mock(ConfigurationModel.class);
  ...
  public void returnsChildConfigurationProviderFromModel() throws Exception {
    ...
    when(childExtensionManager.getConfigurationProvider(childExtension, configurationModel))
        .thenReturn(of(childConfigurationProvider));
    when(parentExtensionManager.getConfigurationProvider(childExtension, configurationModel)).thenReturn(empty());

    Optional<ConfigurationProvider> configurationProvider =
        extensionManager.getConfigurationProvider(childExtension, configurationModel);
  ...
  public void returnsParentConfigurationProviderFromModel() throws Exception {
    ...
    when(parentExtensionManager.getConfigurationProvider(parentExtension, configurationModel))
        .thenReturn(of(parentConfigurationProvider));
    when(childExtensionManager.getConfigurationProvider(parentExtension, configurationModel)).thenReturn(empty());

    Optional<ConfigurationProvider> configurationProvider =
        extensionManager.getConfigurationProvider(parentExtension, configurationModel);
  ...
  public void returnsConfigurationFromModel() throws Exception {
    ...
    ConfigurationInstance configuration = extensionManager.getConfiguration(childExtension, configurationModel, event);
  ...
  public void failsToObtainMissingConfigurationFromModel() throws Exception {
    ...
    ConfigurationInstance configurationInstance = mock(ConfigurationInstance.class);
    when(childConfigurationProvider.get(event)).thenReturn(configurationInstance);
    when(childExtensionManager.getConfigurationProvider(childExtension, configurationModel)).thenReturn(empty());
    when(parentExtensionManager.getConfigurationProvider(childExtension, configurationModel)).thenReturn(empty());

    expectedException.expect(IllegalArgumentException.class);
    extensionManager.getConfiguration(childExtension, configurationModel, event);
  }
